...
function loadInventory() {
  ...
  Papa.parse(INVENTORY_CSV_URL + '&_t=' + Date.now(), {
    ...
    complete: function (results) {
      ...
      // Stop at the first row that contains the standalone marker "sold"
      // (the green divider bar in the sheet) — everything from that row
      // down is excluded.
      const cutoffIndex = allRows.findIndex(row =>
        Object.values(row).some(v => String(v || '').trim().toLowerCase().includes('sold'))
      );
      const rows = cutoffIndex === -1 ? allRows : allRows.slice(0, cutoffIndex);
